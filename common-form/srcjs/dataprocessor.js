function getChoiceData(choiceCount, prevWritten) {
  ...
  for (
    let index = prevWritten;
    index < mainpageElements1.mcqChoices.length;
    index++
  ) {
    ...
    choiceCount--;
    if (choiceCount === 0) {
      mainpageElements1.globalWrittenCount = ++index;
      break;
    }
  }
...
mainpageElements1.button.addEventListener("click", function (e) {
  ...
  mainpageElements1.mcqQuestions.forEach((elm, ind) => {
    ...
    getChoiceData(
      mainpageElements1.mcqChoiceCounts[ind],
      mainpageElements1.globalWrittenCount
    );
  });
...
mainpageElements1.editAccessBtn.addEventListener("click", () => {
  animateCommon(
    [mainpageElements1.imputPassCont],
...
mainpageElements1.passConfirm.addEventListener("click", () => {
  ...
  let formPass = dataGotten.FormData[2];

  if (givenPass === formPass) {
    animateCommon(
      [mainpageElements1.editOnly],
      ["height:100px; padding:7px ;margin-top:-100px"]
    );
    animateCommon(
      [mainpageElements1.editAccessCont],
...
mainpageElements1.submitUserSide.addEventListener("click", function (e) {
  ...
  if (checkViability) {
    uploadData("data", [currentSeed, dataToSend]);
  }
});
